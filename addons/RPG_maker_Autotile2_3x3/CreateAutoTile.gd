...
@onready var ultimate_dialog				= $ultimate_revision_dialog/Window
@onready var ultimate_dialog_itemlist	= $ultimate_revision_dialog/Window/HBoxContainer/VBoxContainer/ItemList
@onready var ultimate_dialog_preview_t	= $ultimate_revision_dialog/Window/HBoxContainer/Panel/Panel4/preview_autotile_selected
@onready var ultimate_dialog_collision	= $ultimate_revision_dialog/Window/HBoxContainer/Panel/Panel/OptionButton2
@onready var ultimate_dialog_percent		= $ultimate_revision_dialog/Window/HBoxContainer/Panel/Panel/SpinBox3
@onready var ultimate_dialog_occlusion	= $ultimate_revision_dialog/Window/HBoxContainer/Panel/Panel/CheckButton2
...
func _ready() -> void:
	...
	set_cursor_sizes()
	horizontal_bar.connect("scroll", set_offset_x)
	vertical_bar.connect("scroll", set_offset_y)
	fill_image_list()
	...
	speed_autotile_dialog_b.get_line_edit().connect("focus_exited",
		select_speed_autotile_dialog_b)
	connect("save_completed", hide_all_dialogs)
...
func update_style_for_input_and_text_boxes(obj):
	...
	for child in children:
			...
			if child.tooltip_text.length() > 0:
				if !child.is_connected("mouse_entered", Callable(self, "show_tooltip")):
					...
					if child.has_signal("button_up"):
						child.connect("button_up", hide_tooltip.bind(child, true))
						child.connect("button_down", hide_tooltip.bind(child, true))
					child.connect("mouse_entered", show_tooltip.bind(child))
					child.connect("mouse_exited", hide_tooltip.bind(child))
					child.connect("gui_input", update_tooltip_position.bind(child))
		...
		if line_edit != null:
			...
			line_edit.add_theme_color_override("caret_color", Color("12a108"))
			line_edit.add_theme_color_override("selection_color", Color("ff9292"))
			if !line_edit.is_connected("focus_entered", Callable(self, "select_all_text")):
				line_edit.connect("focus_entered", select_all_text.bind(line_edit))
				line_edit.connect("focus_exited", deselect_text.bind(line_edit))
				if child is SpinBox:
					line_edit.connect("focus_exited", validate_spinbox.bind(child))
...
#	# Variation zoom #1
#	var zoom = Vector2()
#	if ir.x * (image_real.scale.x + value) > tcrp.size.x:
#		zoom.x = max(0.05, min(image_real.scale.x + value, 10))
#	else:
#		if ir.x < tcrp.size.x:
#			zoom.x = image_real.scale.x
#		else:
#			zoom.x = tcrp.size.x / ir.x
#	if ir.y * (image_real.scale.y + value) > tcrp.size.y:
#		zoom.y = max(0.05, min(image_real.scale.y + value, 10))
#	else:
#		if ir.y < tcrp.size.y:
#			zoom.y = image_real.scale.y
#		else:
...
func fit_image(fit_width = true, fit_height = true):
	...
	#image_real.scale = Vector2(zoom_x, zoom_y)
	set_tiles()
	set_max_scroll_in_scroll_bars()
	set_cursor_sizes()
...
func _on_save_all_Button_button_up() -> void:
	...
	for key in data.keys():
		...
		panel.connect("deleted", delete_tilemap_in_save_dialog)
		panel.connect("show_ultimate_revision_dialog_request", show_ultimate_revision_dialog)
		save_dialog_container_vbox.add_child(panel)
		var lineedit = panel.columns_spinBox.get_line_edit()
		lineedit.connect('focus_entered',
			save_dialog_on_focus_changed)
		lineedit.connect('focus_exited',
			save_dialog_on_focus_exited.bind(lineedit))
		panel.tilemap_name_lineEdit.connect('focus_entered',
			save_dialog_on_focus_changed)
		panel.tilemap_name_lineEdit.connect('focus_exited',
			save_dialog_on_focus_exited.bind(panel.tilemap_name_lineEdit))
		panel.tileset_name_lineEdit.connect('focus_entered',
			save_dialog_on_focus_changed)
		panel.tileset_name_lineEdit.connect('focus_exited',
			save_dialog_on_focus_exited.bind(panel.tileset_name_lineEdit))
		var data2 = {
			"tile_width"		: key[0],
			"tile_height"		: key[1],
...
func save_all(user_data = null) -> void:
	...
	for panel in save_panels:
		...
		if !DirAccess.dir_exists_absolute(panel_data.data.autotile_path +
				panel_data.tilemap_name + "/"):
			DirAccess.make_dir_recursive_absolute(panel_data.data.autotile_path +
				panel_data.tilemap_name + "/")
		if !DirAccess.dir_exists_absolute(panel_data.data.tilemap_path):
			DirAccess.make_dir_recursive_absolute(panel_data.data.tilemap_path)
		if !DirAccess.dir_exists_absolute(panel_data.data.tileset_path):
			DirAccess.make_dir_recursive_absolute(panel_data.data.tileset_path)
		...
		if panel_data.single_image:
			...
			size.y = max_height
			# fill image and save
			var img = Image.create(min(Image.MAX_WIDTH, size.x*2),
			...
			for arr in tile_images:
				...
				for img_data in arr:
					...
					if img_data.img is Array:
						...
						var img = Image.create(w * img_data.img.size(), h,
		...
		for id in ids:
			...
			if rect.collision_type == 0: # NO COLLISION
				...
				header += "[sub_resource type=\"ConvexPolygonShape2D\" id=%s]\n" % \
					resource_id
				header += "points = PackedVector2Array( %s )\n\n" % pol
				if rect.type == -1:
...
func _create_collision_polygon(path : String, src_rect: Rect2) -> Array:
	...
	var img = Image.create(src_rect.size.x, src_rect.size.y, false, tex.get_image().get_format())
	img.blit_rect(tex.get_image(), src_rect, Vector2.ZERO)
	tex = ImageTexture.create_from_image(img)
	var bm = BitMap.new()
	bm.create_from_image_alpha(tex.get_image())
	var rect = Rect2(0, 0, tex.get_image().get_width(), tex.get_image().get_height())
...
func get_final_file_name(file_name, ext, path):
	...
	var regex = RegEx.new()
	regex.compile("(.*_)(\\d+)$")
	while FileAccess.file_exists(path + file_name + ext):
...
func _process(delta: float) -> void:
	if final_zoom != image_real.scale:
		...
		change_zoom(zoom)
#		image_real.scale = image_real.scale.move_toward(final_zoom,
#			zoom_timer * delta)
		zoom_timer += 1
		if image_real.scale.distance_to(final_zoom) < 0.001:
			#image_real.scale = final_zoom
			zoom_timer = 10
