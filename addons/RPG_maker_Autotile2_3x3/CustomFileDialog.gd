...
@export_dir var initial_folder: String = "res://"
@export var valid_files: Array[String] = ["png"]
@export var allow_multiple_selection: bool = false
@export var allow_external_files: bool = false
@export var directory_mode: bool = false
...
func _ready() -> void:
	if !is_connected("visibility_changed", _on_CustomFileDialog_visibility_changed):
		connect("visibility_changed", _on_CustomFileDialog_visibility_changed)
	fill_drive_letter()
...
func fill_drive_letter():
	var drives = DirAccess.get_drive_count()
	for i in drives:
		driver_selector.add_item(DirAccess.get_drive_name(i))
...
func _process(delta: float) -> void:
	if process_file_stack.size() > 0:
		...
		for i in _to:
			...
			Grid.add_child(obj)
			obj.connect("change_directory", change_directory)
			obj.connect("dblClick", select_file_by_dblClick)
			obj.connect("selected", select_file)
			current_id += 1
...
func select_drive_letter_from(_path):
	...
	var index = 0
	for i in drives:
		if DirAccess.get_drive_name(i) == drive:
			index = i
			break
...
func populate_files():
	current_id = 0
	if !DirAccess.dir_exists_absolute(initial_folder):
