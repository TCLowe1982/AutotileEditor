...
func _ready() -> void:
	var normal_font = get_theme_font("normal_font", "RichTextLabel")
	if label and normal_font:
...
func set_tooltip_text(_text):
	...
	for i in array_text.size():
		var size_text = font.get_string_size(array_text[i])
		w = max(w, size_text.x)
		h += size_text.y
	size = Vector2(w, h)
