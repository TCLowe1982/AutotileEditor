...
@export var pattern: Texture2D:
	set(value):
		update_image(value)
@export_enum("Horizontal", "Vertical") var scrollbar_design: String:
	set(value):
		change_design(value)
...
func _ready() -> void:
	if !is_connected("item_size_changed", _on_item_rect_changed):
		connect("item_size_changed", _on_item_rect_changed)
